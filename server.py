...
DB = ROOT / "server" / "spns_rapports.db"
...
def replace_table(table_name, items):
    conn = get_db()
    ...
    for item in items:
        ...
        payload = dict(item)
        payload.setdefault("createdAt", timestamp)
        payload["updatedAt"] = payload.get("updatedAt") or timestamp
        rows.append(
            (
                payload["id"],
                json.dumps(payload, ensure_ascii=False),
    ...
    conn.executemany(
        f"insert into {table_name}(id, payload, created_at, updated_at) values(?, ?, ?, ?)",
        rows,
    )
    conn.commit()
    conn.close()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/bootstrap":
            return self.json({"activities": read_table("activities"), "requests": read_table("requests")})
    ...
    def do_POST(self):
        ...
        if path == "/api/activities/replace":
            replace_table("activities", body.get("items", []))
            return self.json({"ok": True})
        if path == "/api/requests/replace":
            replace_table("requests", body.get("items", []))
            return self.json({"ok": True})
